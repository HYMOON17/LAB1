...
model_gx.load_state_dict(torch.load("./trained_models/noise2inverseNEW.pt", map_location='cpu'))
...
losses = []
train_rmses = []
result_path = "Results/ours_ssrl_noise2inverseNEW"
...
for epoch in range(num_epoch):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        net_output = model(noisy_images_odd)

        with torch.no_grad():
            net_output_full = model(noisy_images)
            net_output_gx = model_gx(noisy_images_even)
            net_output_gx_full = model_gx(noisy_images)

        net_output_sum = (net_output + net_output_gx) / 2.0

        loss = loss_function(net_output_sum, noisy_images_even)

        denoised = (net_output_full + net_output_gx_full) / 2.0
